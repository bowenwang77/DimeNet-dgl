...
file_name="main_coord_expnFalse_full"
relax_file_name = "main_coord_expnFalse_relax_full"
doping_file = pandas.read_csv('dataset/csv_APR_11/'+file_name+'.csv')
doping_file = doping_file.loc[(doping_file['element']!='O')].loc[(doping_file['element']!='C')]
num_rows = doping_file.shape[0]
lattice=doping_file.loc[:,'lattice0':'lattice8'].to_numpy()
Pos=doping_file.loc[:,'dop0x':'Cu70z'].to_numpy().reshape(num_rows,74,3)
...
Cartisan_pos = Pos
doping_file_relax = pandas.read_csv('dataset/csv_APR_11/'+relax_file_name+'.csv')
doping_file_relax = doping_file_relax.loc[(doping_file_relax['element']!='O')].loc[(doping_file_relax['element']!='C')]
relax_num_rows = doping_file_relax.shape[0]
Cartisan_pos_relax = doping_file_relax.loc[:,'dop0x':'Cu70z'].to_numpy().reshape(num_rows,74,3)
