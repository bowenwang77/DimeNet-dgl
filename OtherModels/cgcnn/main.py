...
parser.add_argument('--lr', '--learning-rate', default=0.002, type=float,
                    metavar='LR', help='initial learning rate (default: '
                                       '0.01)')
parser.add_argument('--lr-milestones', default=[50,100,150,200,250,300], nargs='+', type=int,
...
def main():
    ...
    logname=args.data_options[0][-10:]+time_stamp
    wandb.init(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch%5==0:
            ...
            # wandb.log({'Mae Err Validate': mae_error,             
            # })
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    if args.task == 'regression':
        print(' {star} MAE {mae_errors.avg:.3f}'.format(star=star_label,
                                                        mae_errors=mae_errors))
        # pdb.set_trace()
        wandb.log({'Mae Err': mae_errors.avg,             
            })
        with open(file=f'cgcnn_test_log.txt', mode='a+') as log:
        ...
        print(' {star} AUC {auc.avg:.3f}'.format(star=star_label,
                                                 auc=auc_scores))
        with open(file=f'cgcnn_test_log.txt', mode='a+') as log:
            log.writelines(' {star} AUC {auc.avg:.3f}\n'.format(star=star_label,
                                                 auc=auc_scores))
